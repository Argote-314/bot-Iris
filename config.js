...
module.exports = kconfig = async (kill, message) => {
	...
	try {
		...
		const side = 2
		var numerkpop = Math.floor(Math.random() * 15) + 1
		...
		switch(command) {
		   ...
		   case 'clima':
			...
			case 'ranking':
				...
				try {
					for (let i = 0; i < 10; i++) {
						...
						board += `${i + 1} *❯⃫✓ ${getTheName}*\n*➦꙱⃫ᗌ _Mensajes_ ➙* ${msgCounter}\n*➦꙱⃫ᗌ _XP_ ➙*${nivel[i].xp}\n*➦꙱⃫ᗌ _Level_ ➙* ${nivel[i].level}\n*➦꙱⃫ᗌ _Patente_ ➙**: ${role}\n*➦꙱⃫ᗌ _I-Coin_ ➙* ${theCrypto}\n\n`
					}
			...
			case 'steal':
				...
				if (lvpc > 1) { await kill.sendTextWithMentions(from, mess.stealwkd(theStealK, stealGain)) } else { await kill.sendTextWithMentions(from, mess.stealfail(theStealK, stealLose)) }
				if (lvpc > 1) { await gaming.addValue(user, Number(stealGain), nivel, 'xp') } else { await gaming.addValue(user, Number(stealLose), nivel, 'xp') }
				if (lvpc > 1) { await gaming.addValue(theStealK, Number(stealLose), nivel, 'xp') } else { await gaming.addValue(theStealK, Number(stealGain), nivel, 'xp') }
				if (noLimits == 0) await gaming.addLimit(user, daily, './lib/config/Gerais/diario.json')
				break
		...
		//pendejadas mias
		case 'memes':
			const memeimg = await axios.get(`https://meme-api.herokuapp.com/gimme/memesmexico`)
			await kill.sendFileFromUrl(from, memeimg.data.url, '', '', id)
			break
			
		case 'kpop':
			await kill.reply(from, mess.wait(), id)
			await kill.sendPtt(from,`./lib/media/audio/kpop/kpop${numerkpop}.mp3`,id)
			break
		
		case 'pregunta':
			var numeropre = Math.floor(Math.random() * 8) + 1
			if (args.length == 0) return await kill.reply(from,'Que quiere consultar?', id)
			if(numeropre == 1){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/no.jpg', 'no.jpg', '', id)
			}
			if(numeropre == 2){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/desdeluego.jpg', 'desdeluego.jpg', '', id)
			}
			if(numeropre == 3){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/talvez.jpg', 'talvez.jpg', '', id)
			}
			if(numeropre == 4){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/despues.jpg', 'despues.jpg', '', id)
			}
			if(numeropre == 5){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/porque.jpg', 'porque.jpg', '', id)
			}
			if(numeropre == 6){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/nose.jpg', 'nose.jpg', '', id)
			}
			if(numeropre == 7){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/probablemente.jpg', 'probablemente.jpg', '', id)
			}
			if(numeropre == 8){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/si.jpg', 'si.jpg', '', id)
			}
			if(numeropre == 9){
				await kill.sendFile(from, './lib/media/img/spiderman_meme/no.jpg', 'no.jpg', '', id)
			}
