		...
		if (isGroupMsg && isxp && !gaming.isWin(user) && !isBlocked) {
			try {
				...
				if (getReqXP(checkLvL) <= haveXptoUp) {
					...
					await kill.reply(from, `*┏͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙͙━━━⟦ _⃫❮ N᤻U᤻E᤻V᤻O᤻⚒N᤻I᤻V᤻E᤻L᤻❯⃦_⟧━━━❭*\n\n*✓⃟⃫᭢/❬N᤻o᤻m᤻bre᤻ :* ⟦⟧ ${pushname}\n\n*➜⃟⛃❬᤻XP᤻ :* ${await gaming.getValue(user, nivel, 'xp')} / ${getReqXP(checkLvL)}\n\n*➦⃟⃫✓᤻❬᤻ N᤻iv᤻el᤻ :* ${checkLvL} -> ${await gaming.getValue(user, nivel, 'level')} 🆙 \n\n*➦⃟⃫✓Coin:* ${await gaming.getValue(user, nivel, 'coin')}\n\n*➦⃟⃫✓᤻❬᤻C᤻la᤻se᤻:* *${patente}*\n\n*❬━━━━━━━━━━━━━━━━━━━━━━❭*\n*¡Felicitaciones por el nivel y habla más (sin inundar) para subir tu rango y XP!* 🎉`, id)
		...
		switch(command) {
		   ...
		   case 'translate':
			...
			case 'rolette':;case 'roleta':
				...
				if(isOwner){side1 = 2} else{ side1 = side}
				side1 == 1 ? await kill.sendFileFromUrl(from, 'https://i.ibb.co/vQj6nq4/roleta1.png', 'rol1.png', mess.loseshot(nrolxp), id) : await kill.sendFileFromUrl(from, 'https://i.ibb.co/PwKR2nR/roleta.jpg', 'rol.jpg', mess.winshot(prolxp), id)
				side1 == 1 ? await gaming.addValue(user, Number(nrolxp), nivel, 'xp') : await gaming.addValue(user, Number(prolxp), nivel, 'xp')
		...
		case 'lizard':
			...
			*/
			...
			case 'kiss':
				...
				await kill.sendStickerfromUrl(from, kis, { author: config.Sticker_Author, pack: config.Sticker_Pack, keepScale: true })
			...
			case 'slap':
				...
				await kill.sendStickerfromUrl(from, slapar, { author: config.Sticker_Author, pack: config.Sticker_Pack, keepScale: true })
			...
			case 'steal':
				...
				if (isOwner){provable_robo = 0} else{provable_robo = 30}
				if (lvpc > provable_robo) { await kill.sendTextWithMentions(from, mess.stealwkd(theStealK, stealGain)) } else { await kill.sendTextWithMentions(from, mess.stealfail(theStealK, stealLose)) }
				if (lvpc > provable_robo) { await gaming.addValue(user, Number(stealGain), nivel, 'xp') } else { await gaming.addValue(user, Number(stealLose), nivel, 'xp') }
				if (lvpc > provable_robo) { await gaming.addValue(theStealK, Number(stealLose), nivel, 'xp') } else { await gaming.addValue(theStealK, Number(stealGain), nivel, 'xp') }
				if (noLimits == 0) await gaming.addLimit(user, daily, './lib/config/Gerais/diario.json')
				break
			...
			case 'bang':
				...
				var disparasticker = ["https://media.giphy.com/media/12ujpD1yKFu0gqKqdA/giphy.gif","https://media.giphy.com/media/Eo1yqs7qumo9FI1Osh/giphy.gif","https://media.giphy.com/media/PPQwRiX0NbgqrGRA7K/giphy.gif","https://media.giphy.com/media/TVlJKeg0N1qcVpfaZF/giphy.gif","https://media.giphy.com/media/WQH1PlAlCy6Ohqjr0K/giphy.gif","https://media.giphy.com/media/W5uwL5ceDIAuIaMru5/giphy.gif"];
				var distick = disparasticker[Math.floor(Math.random() * disparasticker.length)];
				await kill.sendStickerfromUrl(from, distick, { author: config.Sticker_Author, pack: config.Sticker_Pack, keepScale: true })
